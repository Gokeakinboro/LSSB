export let get_post_item = async function (reqObj, model, helpers) {



    let db_get_response = await model.get({ 
        $where: reqObj.payloadData.$where,
        $check_connection: { '$postLikes': reqObj.payloadData.user_id }
    });

    console.log('Get Post Item db_response -===>', db_get_response, reqObj.payloadData, '\n $check res --->>', db_get_response.check_connection_result);
    ...
    if (db_get_response && db_get_response.doc) {
        ...
        const docWithProjections = (({ $extras$, $last_edited_on$, $last_edited_on, $t$, ...o }) => o)(db_get_response.doc) // remove b and c
