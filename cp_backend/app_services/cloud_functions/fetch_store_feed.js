...
export let fetch_store_feed = async function (reqObj, model, helpers) {



    let db_get_response = await model.get({ 

        // $where: { 'attr.is_media_post': true}, 
        $skip: reqObj.payloadData.$page == 0 ? 0 : (reqObj.payloadData.$page * 6),
        $limit: 6,
    ...
    console.log( 
         ...
         'Get Store feed db_response -===>', 'db_get_response', 
         reqObj.payloadData, '\n $check res --->>', 
         db_get_response
