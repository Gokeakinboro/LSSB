...
const { create_store_item } = await import('../cloud_functions/create_store_item.js');

const { fetch_store_feed } = await import('../cloud_functions/fetch_store_feed.js');

const { get_store_item } = await import('../cloud_functions/get_store_item.js');
...
const cross_store_model = new SqyDB_Model({  // db: config._db,  
...
export let theController = async function (reqObj) {
    ...
    try {
        ...
        if ( typeof controller_fncs[cloud_action] == 'function' ) {
            ...
            let cloud_response = await controller_fncs[cloud_action](reqObj, cross_store_model, { utils, Crypto, aNode, auth$});
