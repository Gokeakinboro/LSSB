...
export let create_grant = async function (reqObj, model, helpers) {
    ...
    let capabilities_passed = false;
     ...
     if ( helpers.auth$.role == 'xSuperLSSBxAdmin' || ( helpers.auth$.ca && helpers.auth$.ca.indexOf('createGrant') > -1 ) ) {
    ...
    console.log(' creating --- -----00---- Grant -===>', reqObj.payloadData, '\n db_set_response --=>>>>', db_set_response);
    ...
    if (db_set_response && db_set_response.msg == 'OK') {
        ...
        return { success: true, statusCode: 200, data: { _id: db_set_response._id, msg: 'Grant Created!' } }
    }

    return { success: false, statusCode: 500, error: { msg: 'Error creating Grant!' } }
}