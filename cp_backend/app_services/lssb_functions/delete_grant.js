...
export let delete_grant = async function (reqObj, model, helpers) {


    // if (reqObj.payloadData.cloud_action == 'create_account') {}

    // console.log(' Using grant --- -----00---- User account -===>', helpers.auth$);
    ...
    if (helpers.auth$.role !== 'xSuperLSSBxAdmin' || (helpers.auth$.ca && helpers.auth$.ca.indexOf('deleteGrant') > -1)) {
    ...
    let delete_this_grant_res = await model.unset($query);

    console.log('  delete_this_grant_res =====================-=====-==-->', delete_this_grant_res);

    // return { data: { msg: 'OK' }, statusCode: 200, success: true };

    if (delete_this_grant_res && delete_this_grant_res.msg == "Doc not found") {

        return { error: { msg: 'Grant not found' }, statusCode: 200, success: false };

    }

    if (delete_this_grant_res && delete_this_grant_res.msg == "OK") {
    ...
    return { success: false, statusCode: 500, error: { msg: 'Error deleting Grant. Try again.' } }
